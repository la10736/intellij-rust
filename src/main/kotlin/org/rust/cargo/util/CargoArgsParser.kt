...
import org.rust.cargo.util.CargoArgsParser.Companion.OptionArgsCountRange.Companion.MANY
import org.rust.cargo.util.CargoArgsParser.Companion.OptionArgsCountRange.Companion.ONE
import org.rust.cargo.util.CargoArgsParser.Companion.OptionArgsCountRange.Companion.ZERO
...
class CargoArgsParser private constructor(
    ...
    private fun getActualOptionArgsCount(optionIdx: Int): Int {
        ...
        val (minArgsCount, maxArgsCount) = optionsToArgsCountRange.getOrDefault(option, ZERO)
    ...
    companion object {
        ...
        data class OptionArgsCountRange(val min: Int, val max: Int) {
            companion object {
        ...
        private val RUN_OPTIONS: Map<String, OptionArgsCountRange> =
            hashMapOf(
                ...
                "--target" to ONE,
                "--target-dir" to ONE,
                "--manifest-path" to ONE,
                "--message-format" to ONE,
                "--color" to ONE,
                "-Z" to ONE
            )
        private val TEST_OPTIONS: Map<String, OptionArgsCountRange> =
            RUN_OPTIONS + hashMapOf("--test" to ONE, "--bench" to ONE, "--exclude" to ONE)
    }
}
